...
public static class EgaisMarkAnalyzer
{
    ...
    {
        var result = new List<ExtractedMark>(); var seen = new HashSet<string>(StringComparer.Ordinal);
        foreach (var candidate in Candidates(raw))
        {
            var normalized = Normalize(candidate); if (string.IsNullOrWhiteSpace(normalized)) continue;
            var parts = Parse(normalized); if (string.IsNullOrWhiteSpace(parts.Type) || string.IsNullOrWhiteSpace(parts.Rank) || string.IsNullOrWhiteSpace(parts.Number)) continue;
            var key = $"{parts.Type}|{parts.Rank}|{parts.Number}"; var duplicate = !seen.Add(key);
    ...
    public static MarkParts Parse(string raw)
    {
        ...
        return digits.Length >= 14 ? new(digits[..3], digits.Substring(3, 3), digits.Substring(6, 8), raw ?? "") : new("", "", "", raw ?? "");
    }
    ...
    static ResponseAnalysis AnalyzeJson(JsonElement root)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status)) { var s = status.ToLowerInvariant(); if (ContainsAny(s, "error", "reject", "fail", "invalid", "ошиб", "отказ")) return new("EGAIS_ERROR", message ?? status!, false, true, false, false); if (ContainsAny(s, "ok", "success", "accepted", "processed", "принят", "обработан", "done", "completed")) return new("OK", message ?? status!, true, false, false, false); }
        if (!string.IsNullOrWhiteSpace(message)) { var m = message.ToLowerInvariant(); if (ContainsAny(m, "error", "ошиб", "отказ", "не найден", "invalid", "reject")) return new("EGAIS_ERROR", message!, false, true, false, false); }
    ...
    {
        var root = doc.Root; if (root == null) return new("UNKNOWN", "Пустой XML-ответ", false, false, false, false);
        ...
        if (errorNode != null) return new("EGAIS_ERROR", ShortMessage(string.IsNullOrWhiteSpace(errorNode.Value) ? errorNode.Name.LocalName : errorNode.Value.Trim()), false, true, false, false);
        ...
        if (!string.IsNullOrWhiteSpace(status)) { var s = status.ToLowerInvariant(); if (ContainsAny(s, "error", "reject", "fail", "invalid", "ошиб", "отказ", "не найден")) return new("EGAIS_ERROR", status!, false, true, false, false); if (ContainsAny(s, "ok", "success", "accepted", "processed", "принят", "обработан", "done", "completed")) return new("OK", status!, true, false, false, false); }
    ...
    {
        var result = new List<string>(); if (string.IsNullOrWhiteSpace(raw)) return result;
        result.AddRange(GSSeparated.Matches(raw).Select(m => m.Value)); result.AddRange(Digits14.Matches(raw).Select(m => m.Value));
        try
        {
            using var j = JsonDocument.Parse(raw); foreach (var s in JsonStrings(j.RootElement)) { result.AddRange(GSSeparated.Matches(s).Select(m => m.Value)); result.AddRange(Any14.Matches(s).Select(m => m.Value)); }
        }
        catch { }
        try
        {
            var x = XDocument.Parse(raw); foreach (var e in x.Descendants().Where(e => LooksLikeMarkField(e.Name.LocalName))) { result.AddRange(GSSeparated.Matches(e.Value).Select(m => m.Value)); result.AddRange(Any14.Matches(e.Value).Select(m => m.Value)); }
        }
        catch { }
        return result;
    }
    ...
    {
        if (e.ValueKind == JsonValueKind.Object) foreach (var p in e.EnumerateObject()) { if (keys.Any(k => p.Name.Equals(k, StringComparison.OrdinalIgnoreCase))) return p.Value.ToString(); var nested = FindValue(p.Value, keys); if (nested != null) return nested; }
    ...
    static string ShortMessage(string s) { var x = Regex.Replace(s.Trim(), @"\s+", " "); return x.Length > 500 ? x[..500] + "…" : x; }
}
