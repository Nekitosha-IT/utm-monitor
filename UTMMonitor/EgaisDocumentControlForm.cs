...
public sealed class EgaisDocumentControlForm : Form
{
    ...
    void LoadDocuments()
    {
        ...
        allDocs = db.Documents(u.Id); ApplyFilter(); state.Text = $"Документов: {allDocs.Count}";
    }
    ...
    void ShowSelected()
    {
        ...
        {
            ID = x.Id, Марка = x.Raw, Type = x.Type, Rank = x.Rank, Number = x.Number, Состояние = FriendlyMarkStatus(x.Status), Проверено = FriendlyDate(x.CheckedAt), Ответ = Short(x.Response, 180)
        }).ToList();
    }
    ...
    void ControlSelected()
    {
        ...
        var id = docs.CurrentRow.Cells[0].Value?.ToString(); var d = allDocs.FirstOrDefault(x => x.Id == id); if (d == null) return;
        var u = SelectedUtm(); if (u == null) return;
        var ms = db.Marks(u.Id).Where(x => x.DocumentId.Equals(d.Id, StringComparison.OrdinalIgnoreCase)).ToList(); var parsed = ParseDocument(d.RawJson);
        var total = parsed.Items.Sum(x => x.Количество); var markCount = parsed.Items.Count(x => !string.IsNullOrWhiteSpace(x.Марка));
        ...
        var errors = ms.Count(x => x.Status.Equals("ERROR", StringComparison.OrdinalIgnoreCase) || x.Status.Equals("TIMEOUT", StringComparison.OrdinalIgnoreCase)); var missing = Math.Max(0, markCount - ms.Count);
    ...
    {
        var u = SelectedUtm(); if (u == null) return "—"; if (string.IsNullOrWhiteSpace(d.Number) || string.IsNullOrWhiteSpace(d.Date)) return "Реквизиты";
        var ms = db.Marks(u.Id).Where(x => x.DocumentId.Equals(d.Id, StringComparison.OrdinalIgnoreCase)).ToList();
        if (ms.Any(x => x.Status.Equals("ERROR", StringComparison.OrdinalIgnoreCase) || x.Status.Equals("TIMEOUT", StringComparison.OrdinalIgnoreCase))) return "Марки: ошибка"; return "OK";
    }
    ...
    static ParsedDocument ParseDocument(string raw)
    {
        ...
        try { using var j = JsonDocument.Parse(raw); ParseJson(j.RootElement, result); if (result.Items.Count > 0) return result; } catch { }
