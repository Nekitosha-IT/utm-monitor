...
public sealed class EgaisDocumentControlForm : Form
{
    ...
    {
        Text = "ЕГАИС — Контроль ТТН и акцизных марок"; Width = 1450; Height = 900; MinimumSize = new(1100, 700); StartPosition = FormStartPosition.CenterParent;
        var root = new TableLayoutPanel { Dock = DockStyle.Fill, RowCount = 2, ColumnCount = 1, Padding = new Padding(10) }; root.RowStyles.Add(new(SizeType.Absolute, 46)); root.RowStyles.Add(new(SizeType.Percent, 100));
        var top = new FlowLayoutPanel { Dock = DockStyle.Fill, WrapContents = false };
        top.Controls.Add(new Label { Text = "УТМ:", AutoSize = true, Padding = new Padding(0, 7, 4, 0) }); top.Controls.Add(utm); top.Controls.Add(new Label { Text = "Поиск:", AutoSize = true, Padding = new Padding(10, 7, 4, 0) }); top.Controls.Add(search);
        top.Controls.Add(Button("Обновить", LoadDocuments)); top.Controls.Add(Button("Проверить ТТН", ControlSelected)); top.Controls.Add(Button("Экспорт проблем", ExportProblems)); top.Controls.Add(state); root.Controls.Add(top, 0, 0);
        var split = new SplitContainer { Dock = DockStyle.Fill, Orientation = Orientation.Horizontal, SplitterDistance = 380 }; split.Panel1.Controls.Add(docs);
        var lower = new TabControl { Dock = DockStyle.Fill }; var tDetails = new TabPage("Реквизиты"); tDetails.Controls.Add(details); var tItems = new TabPage("Товары"); tItems.Controls.Add(items); var tMarks = new TabPage("Марки"); tMarks.Controls.Add(marks); lower.TabPages.Add(tDetails); lower.TabPages.Add(tItems); lower.TabPages.Add(tMarks); split.Panel2.Controls.Add(lower); root.Controls.Add(split, 0, 1); Controls.Add(root);
        Load += (_, _) => { LoadUtms(); LoadDocuments(); }; search.TextChanged += (_, _) => ApplyFilter(); docs.SelectionChanged += (_, _) => ShowSelected();
    }
    ...
    void LoadDocuments() { var u = SelectedUtm(); if (u == null) return; allDocs = db.Documents(u.Id); ApplyFilter(); state.Text = $"Документов: {allDocs.Count}"; }

    void ApplyFilter()
    {
        var q = search.Text.Trim(); var rows = allDocs.Where(d => string.IsNullOrWhiteSpace(q) || (d.Id + " " + d.Number + " " + d.Type + " " + d.Direction + " " + d.Status + " " + d.RawJson).Contains(q, StringComparison.OrdinalIgnoreCase)).Select(d => new
        {
            ID = d.Id, Направление = FriendlyDirection(d.Direction), Документ = FriendlyType(d.Type), Номер = string.IsNullOrWhiteSpace(d.Number) ? "—" : d.Number,
            Дата = FriendlyDate(d.Date), Состояние = FriendlyStatus(d.Status), Позиций = d.Items, Контроль = DocumentState(d)
        }).ToList();
        docs.DataSource = rows; if (docs.Rows.Count > 0) docs.Rows[0].Selected = true;
    }

    void ShowSelected()
    {
        if (docs.CurrentRow == null) return; var id = docs.CurrentRow.Cells[0].Value?.ToString(); var d = allDocs.FirstOrDefault(x => x.Id == id); if (d == null) return; var u = SelectedUtm(); if (u == null) return;
        var parsed = ParseDocument(d.RawJson); details.SelectedObject = new DocumentProperties(d, parsed); items.DataSource = parsed.Items;
        marks.DataSource = db.Marks(u.Id).Where(x => x.DocumentId.Equals(d.Id, StringComparison.OrdinalIgnoreCase)).Select(x => new { ID = x.Id, Марка = x.Raw, Type = x.Type, Rank = x.Rank, Number = x.Number, Состояние = FriendlyMarkStatus(x.Status), Проверено = FriendlyDate(x.CheckedAt), Ответ = Short(x.Response, 180) }).ToList();
    }

    void ControlSelected()
    {
        if (docs.CurrentRow == null) { MessageBox.Show(this, "Выберите ТТН."); return; }
        var id = docs.CurrentRow.Cells[0].Value?.ToString(); var d = allDocs.FirstOrDefault(x => x.Id == id); var u = SelectedUtm(); if (d == null || u == null) return;
        using var form = new EgaisTtnMarkCheckForm(u, d); form.ShowDialog(this); LoadDocuments();
    }

    void ExportProblems()
    {
        var u = SelectedUtm(); if (u == null) return; var problems = allDocs.Where(d => DocumentState(d) != "OK").ToList();
        ...
        using var sw = new StreamWriter(dlg.FileName, false, new System.Text.UTF8Encoding(true)); sw.WriteLine("ID;Направление;Тип;Номер;Дата;Статус;Позиции;Контроль");
    ...
    string DocumentState(DocumentRow d)
    {
        ...
        var discovered = EgaisMarkAnalyzer.ExtractMarks(d.RawJson); var unique = discovered.Where(x => !x.Duplicate).ToList();
        var duplicateCount = discovered.Count(x => x.Duplicate);
        var ms = db.Marks(u.Id).Where(x => x.DocumentId.Equals(d.Id, StringComparison.OrdinalIgnoreCase)).ToList();
        if (duplicateCount > 0) return "Дубли марок";
        if (unique.Count == 0) return "Марки не найдены";
        ...
        if (ms.Any(x => x.Status.Equals("UNKNOWN", StringComparison.OrdinalIgnoreCase))) return "Не распознано";
        var keys = ms.Select(x => $"{x.Type}|{x.Rank}|{x.Number}").ToHashSet(StringComparer.OrdinalIgnoreCase);
        var successful = ms.Where(x => x.Status.Equals("OK", StringComparison.OrdinalIgnoreCase) || x.Status.Equals("SUCCESS", StringComparison.OrdinalIgnoreCase)).Select(x => $"{x.Type}|{x.Rank}|{x.Number}").ToHashSet(StringComparer.OrdinalIgnoreCase);
        if (unique.Any(x => !keys.Contains($"{x.Parts.Type}|{x.Parts.Rank}|{x.Parts.Number}"))) return "Не проверено";
        if (unique.Any(x => !successful.Contains($"{x.Parts.Type}|{x.Parts.Rank}|{x.Parts.Number}"))) return "Не подтверждено";
        return "OK";
    }
    ...
    static string FriendlyMarkStatus(string s) => s switch { "OK" or "SUCCESS" => "✓ OK", "ERROR" => "✕ Ошибка", "TIMEOUT" => "⌛ Таймаут", "UNKNOWN" => "? Не распознано", _ => string.IsNullOrWhiteSpace(s) ? "Не проверена" : s };
    ...
    {
        var result = new ParsedDocument(); try { using var j = JsonDocument.Parse(raw); ParseJson(j.RootElement, result); if (result.Items.Count > 0) return result; } catch { }
        try { var root = XDocument.Parse(raw).Root; if (root != null) ParseXml(root, result); } catch { } return result;
    }
    static void ParseJson(JsonElement root, ParsedDocument result)
    {
        foreach (var p in new[] { "items", "positions", "products", "productItems", "product" }) if (Find(root, p, out var ar) && ar.ValueKind == JsonValueKind.Array) { var i = 0; foreach (var x in ar.EnumerateArray()) result.Items.Add(new ItemView(++i, J(x, "name", "productName", "fullName", "goodsName"), N(x, "quantity", "qty", "amount", "count"), N(x, "price", "sum", "cost"), J(x, "mark", "barcode", "exciseMark", "markCode", "barcodeDataMatrix", "amc"))); return; }
    }
