...
public sealed class UtmDatabase
{
    ...
    public UtmDatabase()
    {
        ...
        {
            using var q = c.CreateCommand(); q.CommandText = "INSERT INTO utms(name,host,port,enabled) VALUES('УТМ 1','127.0.0.1',8080,1)"; q.ExecuteNonQuery();
        }
    }
    ...
    public void Delete(int id)
    {
        ...
        using (var q = c.CreateCommand()) { q.Transaction = tx; q.CommandText = "DELETE FROM utms WHERE id=$id"; q.Parameters.AddWithValue("$id", id); q.ExecuteNonQuery(); }
        tx.Commit();
    }
    ...
    public void SaveDocuments(int uid, IEnumerable<DocumentRow> rows)
    {
        ...
        foreach (var d in rows)
        {
            ...
            q.CommandText = @"INSERT INTO documents(utm_id,external_id,type,direction,number,date,status,items,raw,updated_at) VALUES($u,$x,$t,$d,$n,$dt,$s,$i,$r,$now)
ON CONFLICT(utm_id,external_id) DO UPDATE SET type=$t,direction=$d,number=$n,date=$dt,status=$s,items=$i,raw=$r,updated_at=$now";
    ...
    static void SaveNested(SqliteConnection c, SqliteTransaction tx, int uid, DocumentRow d)
    {
        try
        {
            ...
            foreach (var key in new[] { "items", "positions", "products", "productItems", "product" })
            {
                ...
                var i = 0;
                foreach (var item in ar.EnumerateArray()) SaveItem(c, tx, uid, d.Id, item, i++);
                return;
            }
        }
        catch (JsonException) { }
        ...
        try
        {
            var root = XDocument.Parse(d.RawJson).Root;
            if (root == null) return;
            ...
            if (items.Count == 0)
            {
                var codes = root.Descendants().Where(e => e.Name.LocalName.Equals("alcCode", StringComparison.OrdinalIgnoreCase)).ToList();
                items = codes.Select(x => x.Parent ?? x).Distinct().ToList();
            }
    ...
    static void SaveItem(SqliteConnection c, SqliteTransaction tx, int uid, string doc, JsonElement item, int index)
    {
        ...
        SaveItemValues(c, tx, uid, doc, index.ToString(CultureInfo.InvariantCulture), Get("name", "productName", "fullName", "shortName", "goodsName", "alcoholName"), ParseNumber(Get("quantity", "qty", "amount", "count")), ParseNumber(Get("price", "sum", "cost")), Get("mark", "barcode", "exciseMark", "markCode", "ean", "barcodeDataMatrix"), item.GetRawText());
    }
    ...
    static void SaveXmlItem(SqliteConnection c, SqliteTransaction tx, int uid, string doc, XElement item, int index)
    {
        ...
        SaveItemValues(c, tx, uid, doc, key, Get("name", "productName", "fullName", "shortName", "goodsName", "alcoholName", "shortName"), ParseNumber(Get("quantity", "qty", "amount", "count")), ParseNumber(Get("price", "sum", "cost", "priceWithVat")), mark, item.ToString(SaveOptions.DisableFormatting));
    }
    ...
    {
        using var q = c.CreateCommand(); q.Transaction = tx;
        q.CommandText = @"INSERT INTO document_items(utm_id,document_external_id,item_key,name,quantity,price,mark,raw,updated_at) VALUES($u,$d,$k,$n,$q,$p,$m,$r,$t)
ON CONFLICT(utm_id,document_external_id,item_key) DO UPDATE SET name=$n,quantity=$q,price=$p,mark=$m,raw=$r,updated_at=$t";
        q.Parameters.AddWithValue("$u", uid); q.Parameters.AddWithValue("$d", doc); q.Parameters.AddWithValue("$k", key); q.Parameters.AddWithValue("$n", name); q.Parameters.AddWithValue("$q", quantity); q.Parameters.AddWithValue("$p", price); q.Parameters.AddWithValue("$m", mark); q.Parameters.AddWithValue("$r", raw); q.Parameters.AddWithValue("$t", DateTime.UtcNow.ToString("O")); q.ExecuteNonQuery();
        if (mark != "") SaveMark(c, tx, uid, doc, mark);
    }
    ...
    static string? XmlValue(XElement root, params string[] names)
    {
        ...
        {
            var a = root.Attributes().FirstOrDefault(x => x.Name.LocalName.Equals(n, StringComparison.OrdinalIgnoreCase));
            if (a != null && !string.IsNullOrWhiteSpace(a.Value)) return a.Value.Trim();
            var e = root.DescendantsAndSelf().FirstOrDefault(x => x.Name.LocalName.Equals(n, StringComparison.OrdinalIgnoreCase));
...
ON CONFLICT(utm_id,document_external_id,raw) DO UPDATE SET type=$t,rank=$k,number=$n,status=$s,response=$v,checked_at=$c";
        q.Parameters.AddWithValue("$u", uid); q.Parameters.AddWithValue("$d", (object?)document ?? DBNull.Value); q.Parameters.AddWithValue("$r", raw); q.Parameters.AddWithValue("$t", p.Type); q.Parameters.AddWithValue("$k", p.Rank); q.Parameters.AddWithValue("$n", p.Number); q.Parameters.AddWithValue("$s", status); q.Parameters.AddWithValue("$v", response); q.Parameters.AddWithValue("$c", DateTime.UtcNow.ToString("O")); q.ExecuteNonQuery();
    }
    ...
    {
        if (e.ValueKind == JsonValueKind.Object) { foreach (var p in e.EnumerateObject()) { if (string.Equals(p.Name, key, StringComparison.OrdinalIgnoreCase)) { value = p.Value; return true; } if (Find(p.Value, key, out value)) return true; } }
        else if (e.ValueKind == JsonValueKind.Array) foreach (var i in e.EnumerateArray()) if (Find(i, key, out value)) return true;
