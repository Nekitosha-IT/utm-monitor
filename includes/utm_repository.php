...
{
    $s = $db->prepare('SELECT * FROM utms WHERE id = ? LIMIT 1');
    $s->execute([$id]);
...
function addUtm(PDO $db, string $name, string $ip, int $port, ?string $externalId = null): int
{
    $name = trim($name);
    $ip = trim($ip);
    if ($name === '' || $ip === '' || $port < 1 || $port > 65535) {
        throw new InvalidArgumentException('Некорректные параметры УТМ');
    }
    $s = $db->prepare('INSERT INTO utms (external_id,name,ip,port,enabled) VALUES (?,?,?,?,1)');
    $s->execute([$externalId ?: null, $name, $ip, $port]);
...
function updateUtm(PDO $db, int $id, string $name, string $ip, int $port): bool
{
    ...
    $s->execute([trim($name), trim($ip), $port, $id]);
    return $s->rowCount() > 0;
}
...
function deleteUtm(PDO $db, int $id): bool
{
    ...
    $s = $db->prepare('DELETE FROM utms WHERE id=?');
    $s->execute([$id]);
    return $s->rowCount() > 0;
}
...
{
    $s = $db->prepare('UPDATE utms SET enabled=?, updated_at=CURRENT_TIMESTAMP WHERE id=?');
    $s->execute([$enabled ? 1 : 0, $id]);
    return $s->rowCount() > 0;
}
